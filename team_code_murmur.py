...
from base_model_murmur import AudioClassifier
from dataset_murmur import SoundDS_Murmur, SoundDS_Patch_Murmur
import utils

# XGBoost for meta-learner
try:
...
def get_features(data, recordings):
    """提取临床特征（用于Outcome分类）- 从原始team_code.py复制"""
    is_pregnant = get_pregnancy_status_mod(data)
    age_group = get_age(data)
...
def train_challenge_model(data_folder, model_folder, verbose):
    ...
    for i, (train_idx, val_idx) in enumerate(kf.split(pIDs, labels)):
        ...
        train_ds = SoundDS_Murmur(df_tr, data_folder, mode='train', df_wide=df_wide_tr, use_segmentation=True)
        val_ds = SoundDS_Patch_Murmur(df_val, data_folder, df_wide=df_wide_val)
        ...
        fold_labels = []
        with torch.no_grad():
            for patches_batch, wide_features, label_batch in val_loader:
                num_patches = patches_batch.shape[1]
                patches = patches_batch.squeeze(0).to(device)
                wide_features_expanded = wide_features.repeat(num_patches, 1).to(device)
                outputs = model(patches, wide_features_expanded)
                ...
                fold_labels.append(label_batch.item())
        
        meta_features_train.extend(fold_preds)
        meta_labels_train.extend(fold_labels)
    ...
    # 训练XGBoost元学习器
    print('\n===== 训练XGBoost元学习器 =====')
    meta_X = np.array(meta_features_train)
    meta_y = np.array(meta_labels_train)
    
    xgb_meta = XGBClassifier(n_estimators=100, max_depth=5, learning_rate=0.1, random_state=42, device='cuda:1')
    xgb_meta.fit(meta_X, meta_y)
    joblib.dump(xgb_meta, os.path.join(model_folder, 'xgb_meta_learner.pkl'))
...
def load_challenge_model(model_folder, verbose):
    ...
    xgb_meta = joblib.load(os.path.join(model_folder, 'xgb_meta_learner.pkl'))
    
    return {'murmur_models': murmur_models, 'outcome_models': outcome_models, 'xgb_meta': xgb_meta}

def run_challenge_model(model, data, recordings, verbose):
    murmur_models = model['murmur_models']
    outcome_models = model['outcome_models']
    xgb_meta = model['xgb_meta']
